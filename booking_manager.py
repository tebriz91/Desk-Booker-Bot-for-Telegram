...
import pytz  # If handling different time zones
import holidays  # If considering public holidays set COUNTRY_CODE in config.py
import config
...
def cancel_button(update: Update, context: CallbackContext) -> None:
...
def date_selected(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        existing_booking = execute_db_query(
        ...
        if existing_booking:
            query.edit_message_text("You have already booked a desk for this date. Choose another date or cancel the existing booking.")
            return

        # Retrieve the list of available rooms from the database
        rooms = execute_db_query(
        ...
        if rooms:
            ...
            query.edit_message_text("No rooms available to book.")
    except Exception as e:
...
def room_selected(update: Update, context: CallbackContext) -> None:
    ...
    try: # Retrieve the room name and plan URL. Only select available rooms
        ...
        if not room_result:
            query.edit_message_text("Selected room is not available. Please choose another room.")
            return
        ...
        if desks:
            ...
            query.edit_message_text(text=text_with_image_link, reply_markup=reply_markup, parse_mode='MarkdownV2')
        else:
            query.edit_message_text("No desks available in the selected room.")
    except Exception as e:
...
def display_bookings_for_cancellation(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        update.message.reply_text(response_text, reply_markup=reply_markup if bookings else None)
    except Exception as e:
...
def cancel_booking(update: Update, context: CallbackContext) -> None:
    ...
    try:
        execute_db_query("DELETE FROM bookings WHERE booking_id = ? AND user_id = ?", (booking_id, user_id))
...
def view_my_bookings(update: Update, context: CallbackContext) -> None:
    ...
    try:
        query = """
            ...
            ORDER BY b.booking_date
        """
        bookings = execute_db_query(query, (user_id, today, next_four_workdays), fetch_all=True)
...
def view_all_bookings(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        """
        bookings = execute_db_query(query, (today, next_four_workdays), fetch_all=True)
